...
#define TIMER_STOP \
    ...
    TEA::time_log(elapsedTime);
const int ENCRYPT = 1, DECRYPT = 0;
unsigned long mem[10000], y = 0;
long long len = 0, i, psize = 0, pos = 0, var = 0, flag;
...
public:
	...
	void BlockTEA(unsigned char *data, char flag)
	{
	...
	static void time_log(double p)
	{
		ofstream t_log;
		t_log.open("Tlog.txt", std::ios::app);
		auto t = std::time(nullptr);
		auto tm = *std::localtime(&t);
		t_log << put_time(&tm, "%d-%m-%Y %H-%M-%S") << " :: " << flag << " length : " << len << " time elapsed :" << p << endl;
	}
